...
private abstract class NodeContainer : Node() {
    ...
    fun add(child: Node,name : String? = null) {
        children.add(child)
        if(name != null)
            searchCache[name] = child as Declaration;
        if (_first == null)
            _first = child
        _last?.next = child
        child.prev = _last
    ...
    val searchCache = mutableMapOf<String,Declaration>();
...
private open class Scope(
    ...
    private fun find(expr: LuaNameExpr): Declaration? {
        val name = expr.name
        var ret: Declaration? = searchCache[name];
        if(ret == null) {
            walkUp(tree.getPosition(expr), 0) {
                if (it.name == name) {
                    ret = it; searchCache[name] = it;false
                } else true
            }
        }
        return ret
